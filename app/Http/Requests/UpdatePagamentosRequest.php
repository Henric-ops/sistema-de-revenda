    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'valor_pago' => [
                ...
                'min:0.01',
                'max:999999.99',
                function ($attribute, $value, $fail) use ($compra, $pagamento) {
                    // Calcular saldo restante considerando o pagamento atual
                    $totalPagado = $compra->pagamentos()
                        ->where('id', '!=', $pagamento->id)
                        ->sum('valor_pago');
                    $novoSaldoRestante = $compra->valor_total - $totalPagado;

                    if ($value > $novoSaldoRestante) {
                        $fail("O valor pago ({$value}) não pode ser maior que o saldo restante ({$novoSaldoRestante}).");
                    }
                },
            ],
            'metodo_pagamento' => [
